...
export default function EditReceptionistPage({
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        setReceptionistId(resolvedParams.id);

        // Fetch receptionist data
        const receptionistRes = await axios.get(
          `/receptionists/${resolvedParams.id}`
        );
        const receptionist = receptionistRes.data.receptionist;
        ...
        setError(
          err.response?.data?.error || "Failed to load receptionist data"
        );
        setLoading(false);
      }
    }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await axios.patch(`/receptionists/${receptionistId}`, {
        name: formData.name,
      ...
      router.push(`/dashboard/receptionists/${receptionistId}`);
      router.refresh();
    } catch (err: any) {
      setError(err.response?.data?.error || "Failed to update receptionist");
      setSubmitting(false);
    }
  }
        ...
        <div className="max-w-2xl mx-auto bg-white rounded-lg shadow-md p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-gray-800">
            ...
            <div className="flex gap-4">
              ...
              <Link
                href={`/dashboard/receptionists/${receptionistId}`}
